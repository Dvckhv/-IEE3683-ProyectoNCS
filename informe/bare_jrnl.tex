...
\begin{algorithm}[!t] 
\caption{User Scheduling (Agendamiento de Usuarios)}
\label{alg:user_scheduling}
\begin{algorithmic}[1]
\renewcommand{\algorithmicrequire}{\textbf{Input:}}
\renewcommand{\algorithmicensure}{\textbf{Output:}}
\REQUIRE Spacing error $|e_{i,p}|$, error bound $e^{th}$, last scheduled time slot $t_{l}$, length of scheduling period $T$.
\ENSURE User scheduling scheme $O_{i}$.
\STATE \textbf{procedure} USER SCHEDULING OF MV $i$ IN A SCHEDULING PERIOD
\STATE Initialize user scheduling scheme $O_{i} = 0$.
\IF {$|e_{i,p}| \le e^{th}$}
    \STATE Break.
\ELSE
    \STATE Initialize allocated number $a = 0$, next desired time slot $t_{n} = 1$, $f_{i}$ according to Eq. (10), $c_{i} = \frac{T}{f_{i}}$.
    \IF {$(t_{l} + c_{i}) \mod T > T$}
        \STATE $t_{n} = (t_{l} + c_{i}) \mod T$.
    \ENDIF
    \WHILE {$a \le f_{i}$ or $t_{n} \le T$}
        \IF {$t_{n}$ can be scheduled}
            \STATE $o_{i}^{t_{n}} = 1$.
            \STATE $a = a + 1$.
            \STATE $t_{n} = t_{n} + c_{i}$.
        \ELSE
            \STATE Initialize candidate time slot array $C$ (Z-search).
            \STATE Update $t_{n}$ as the first available time slot in $C$.
        \ENDIF
    \ENDWHILE
\ENDIF
...
Una vez fijados los usuarios por slot, se resuelve la asignación de potencia. A diferencia de los sistemas celulares convencionales, se aplica el \textbf{Teorema 1} de \cite{He2023}: \textit{el orden óptimo de decodificación SIC es el orden ascendente de la ganancia del canal de interferencia}.

Para un par de usuarios $j$ y $k$ que comparten un slot, donde el usuario $j$ sufre menos interferencia ($h_{j, k-1} > h_{j-1, k}$) y es decodificado primero, las potencias se calculan de manera ``voraz'' (greedy) para satisfacer estrictamente el umbral $R^{th}$ de tasa mínima:
\begin{align}
