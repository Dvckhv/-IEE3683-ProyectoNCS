...
\textit{jerk}, mientras que las Figuras~\ref{fig:new_model_speeds} y
\ref{fig:new_model_absolute_error} muestran, respectivamente, la velocidad de los
vehículos y el error absoluto promedio. La perturbación introducida en la trayectoria del
líder genera una onda de desaceleración que se propaga hacia la cola del
\emph{platoon}, pero dicha onda es suave: los errores de espaciamiento presentan
...
tercer orden permiten que el \emph{platoon} recupere la velocidad de crucero sin
oscilaciones de alta frecuencia. Finalmente, en la
Figura~\ref{fig:new_model_packets} se presenta la evolución de los paquetes acumulados
recibidos correctamente a lo largo del horizonte de simulación.
...
(Fig.~\ref{fig:paper_model_spacing_error}) muestran trayectorias más irregulares, con
componentes positivas y negativas que se estabilizan en un error estacionario mayor, del
orden de $0.4\,\mathrm{m}$, como se aprecia en la
...
Desde el punto de vista de estabilidad, ambos modelos logran mantener el \emph{platoon}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{Imagenes/paper_model_route_info}
    \caption{Evolución del error de espaciamiento, velocidades y error promedio
    absoluto para el modelo cinemático original.}
    \label{fig:paper_route_info}
\end{figure}
...
las transmisiones son más regulares en el tiempo: una vez que la perturbación alcanza a
...
En el modelo cinemático de referencia, en cambio, se observan ráfagas de transmisión
concentradas en las zonas donde los errores crecen rápidamente, seguidas de intervalos
prolongados prácticamente sin paquetes. El esquema de agendamiento asigna recursos cuando
los errores ya son altos, pero luego puede pasar varios segundos sin nuevas
actualizaciones, lo que contribuye a los cambios abruptos en las acciones de control y a
un error estacionario mayor. El resultado es un uso de canal menos homogéneo en el
tiempo, con picos de demanda y periodos ociosos.
...
paquetes transmitidos se reduce aproximadamente en un $15\%$ (de $3152$ a $2664$), y la
energía de transmisión integrada disminuye alrededor de un $17\%$ (de
$8.15\times10^{-6}\,\mathrm{J}$ a $6.79\times10^{-6}\,\mathrm{J}$). Esto indica que el
...
\begin{table}[H]
    ...
    \begin{tabular}{lcc}
        ...
        \hline
        Cinemático (paper) & $3152$ & $8.15\times10^{-6}$ \\
