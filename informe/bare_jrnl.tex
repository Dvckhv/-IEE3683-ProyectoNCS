...
\title{Joint Platoon Control and Resource Allocation for NOMA-V2V Communication System: A Comparative Analysis of Vehicle Dynamics Impact}
...
\begin{abstract}
El \textit{platooning} vehicular cooperativo, apoyado en comunicaciones V2V fiables, es crucial para los Sistemas de Transporte Inteligente modernos. Es por esto que tanto la gestión eficiente de recursos de comunicación y las recientes arquitecturas NOMA, es vital para mantener la estabilidad del pelotón bajo restricciones de red realistas. Este trabajo analizó el efecto de incorporar un modelo vehicular de tercer orden con limitación explícita de \textit{jerk} dentro de un marco conjunto de control de \textit{platooning} y comunicaciones V2V basado en NOMA.
Se replicó íntegramente la arquitectura de un modelo cinemático simplificado y se comparó su desempeño frente a un nuevo modelo dinámico bajo idénticas condiciones de red y perturbación del líder.
Los resultados mostraron que el modelo con \textit{jerk} acotado produjo trayectorias significativamente más suaves en aceleración, velocidad y espaciamiento, reduciendo las oscilaciones transitorias y mejorando la estabilidad en régimen permanente.
A pesar de su mayor realismo dinámico, el modelo propuesto requirió un menor número de transmisiones y redujo el consumo total de energía en comunicaciones en un 19\% y 25 \% respectivamente, evidenciando que acciones de control más suaves pueden disminuir la demanda sobre el canal. 
En conjunto, los resultados demostraron que NOMA se mantuvo eficaz bajo un modelo vehicular más realista y destacaron la importancia de considerar límites físicos de actuadores en estudios de co-diseño entre control y comunicaciones.
\end{abstract}

\begin{IEEEkeywords}
NOMA-V2V, Platooning, Vehicular Dynamics, Jerk-Limited Control, Distributed Control, Resource Allocation.
\end{IEEEkeywords}
...
Este modelo permite cambios de aceleración tipo escalón, lo que implica un \textit{jerk} infinito teórico y fuerzas instantáneas imposibles de generar por un motor real.
...
\subsubsection{Escenario de prueba y transferencia de la trayectoria del líder}

Para comparar de forma justa el desempeño del modelo cinemático original y del modelo dinámico con \textit{jerk} acotado, se adopta un esquema de testeo en dos etapas. En una primera simulación se ejecuta únicamente el modelo dinámico de tercer orden, utilizando al líder como generador de trayectoria.
Como la entrada de control del líder ya no es la aceleración longitudinal $u_0(t)$, sino el \textit{jerk}, la perturbación se introduce como un \textit{jerk} constante y negativo durante un intervalo, seguido por un lazo de control PD sencillo sobre la velocidad.

Más precisamente, el líder parte en régimen estacionario a velocidad de crucero $v_{\mathrm{cruise}}$. Durante un breve intervalo inicial se mantiene $j_0(t)=0$ para preservar ese estado. 
A continuación, en la ventana temporal de perturbación se fija un \textit{jerk} constante $j_0(t)= \frac{j_{\min}}{10}<0$, lo que produce una reducción suave de la fuerza longitudinal y, por ende, una desaceleración progresiva del líder sin saltos bruscos de aceleración. Finalizado este pulso, se cierra un controlador PD en el dominio de la fuerza
\[
\dot F_0(t) = K_p\bigl(v_{\mathrm{cruise}} - v_0(t)\bigr) - K_d\,a_0(t),
\]

Durante esta primera simulación se registra la aceleración longitudinal del líder en cada instante de muestreo, construyendo una secuencia discreta. 
Al finalizar, dicha secuencia se almacena en un archivo externo que actúa como “tabla de consulta” de la trayectoria del líder.

En una segunda simulación se ejecuta el modelo cinemático de referencia, idéntico al utilizado en el trabajo base, pero forzando al líder a reproducir exactamente el mismo perfil de aceleración generado por el modelo dinámico. 
En cada paso de simulación se aplica directamente como entrada de control cinemática la aceleración previamente registrada.

Con esta estrategia, ambos modelos son sometidos a una perturbación externa idéntica en términos de aceleración y velocidad del líder, pero cada uno la procesa según su propia dinámica interna. La comparación de errores de espaciamiento, esfuerzo de control y métricas de comunicación se realiza así sobre un escenario común, evitando sesgos asociados a trayectorias del líder distintas y aislando el efecto de pasar de un modelo cinemático ideal a uno dinámico con \textit{jerk} limitado.
...
En la Figura~\ref{fig:new_model_spacing_error} se muestra la evolución del error de
espaciamiento por vehículo para el modelo dinámico con limitación explícita de
\textit{jerk}, mientras que las Figuras~\ref{fig:new_model_speeds} y \ref{fig:new_model_absolute_error} muestran, respectivamente, la velocidad de los
vehículos y el error absoluto promedio. La perturbación introducida en la trayectoria del
líder genera una onda de desaceleración que se propaga hacia la cola del pelotón, pero dicha onda es suave: los errores de espaciamiento presentan
variaciones continuas y sin esquinas, alcanzan un máximo cercano a $0.5\,\mathrm{m}$ de manera agregada y de $1.2\,\mathrm{m}$ en el peor vehiculo, 
luego decrecen de forma monótona hasta estabilizarse en un valor estacionario del orden
de los centímetros. La curva de velocidades es igualmente suave, con perfiles casi
superpuestos entre vehículos, lo que indica que el controlador local y la dinámica de
tercer orden permiten que el pelotón recupere la velocidad de crucero sin oscilaciones de alta frecuencia. Finalmente, en la Figura~\ref{fig:new_model_packets} se presenta la evolución de los paquetes acumulados recibidos correctamente a lo largo del horizonte de simulación.

\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{Imagenes/new_model_spacing_error.png}
    \caption{Evolución del error de espaciamiento por vehículo (modelo dinámico).}
    \label{fig:new_model_spacing_error}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{Imagenes/new_model_speeds.png}
    \caption{Evolución de la velocidad por vehículo (modelo dinámico).}
    \label{fig:new_model_speeds}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{Imagenes/new_model_absolute_error.png}
    \caption{Evolución del error de espaciamiento absoluto promedio (modelo dinámico).}
    \label{fig:new_model_absolute_error}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{Imagenes/new_model_packets.png}
    \caption{Paquetes acumulados recibidos correctamente (modelo dinámico).}
    \label{fig:new_model_packets}
\end{figure}

En contraste, las Figuras~\ref{fig:paper_model_spacing_error} y
\ref{fig:paper_model_speeds} corresponden al modelo cinemático original del
\emph{paper}. En este caso, los cambios de aceleración son abruptos y se traducen en
perfiles de velocidad con tramos fuertemente angulados, evidenciando saltos en la
aceleración aplicada a los vehículos. Los errores de espaciamiento
(Fig.~\ref{fig:paper_model_spacing_error}) muestran trayectorias que se estabilizan en un error estacionario mayor, del
orden de $0.2\,\mathrm{m}$ frente a $0.1\,\mathrm{m}$ del modelo dinámico, como se aprecia en la
Figura~\ref{fig:paper_model_absolute_error}. Además, la propagación de la perturbación es
menos suave y tiende a dejar diferencias persistentes entre vehículos.

Desde el punto de vista de estabilidad, ambos modelos logran mantener el pelotón
cohesionado ante la perturbación, pero el modelo dinámico con \textit{jerk} acotado
presenta una respuesta claramente más amortiguada: el máximo error de espaciamiento es
similar o ligeramente menor, el error absoluto promedio decrece más rápido después del
tránsito de la onda y el estado estacionario presenta menores desalineamientos entre
vehículos. Al mismo tiempo, la ausencia de esquinas en las trayectorias de velocidad y
espaciamiento refleja un comportamiento físicamente más realista y confortable para los
pasajeros.

\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{Imagenes/paper_model_spacing_error.png}
    \caption{Evolución del error de espaciamiento por vehículo (modelo cinemático).}
    \label{fig:paper_model_spacing_error}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{Imagenes/paper_model_speeds.png}
    \caption{Evolución de la velocidad por vehículo (modelo cinemático).}
    \label{fig:paper_model_speeds}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{Imagenes/paper_model_absolute_error.png}
    \caption{Evolución del error de espaciamiento absoluto promedio (modelo cinemático).}
    \label{fig:paper_model_absolute_error}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{Imagenes/paper_model_packets.png}
    \caption{Paquetes acumulados recibidos correctamente (modelo cinemático).}
    \label{fig:paper_model_packets}
\end{figure}


\subsubsection{Costo en comunicaciones}

Las Figuras~\ref{fig:new_tx_schedule} y~\ref{fig:paper_tx_schedule} muestran el patrón
de transmisiones para cada seguidor en ambos modelos. En el modelo dinámico propuesto,
las transmisiones son casi regulares en el tiempo: una vez que la perturbación alcanza a
cada vehículo, éste comienza a transmitir de manera casi periódica a lo largo del
horizonte. El agendamiento NOMA distribuye las oportunidades de transmisión de forma
uniforme entre los seguidores, evitando grandes intervalos sin comunicación. Esta
regularidad es coherente con los perfiles de error más suaves, que requieren ajustes
graduales de control y, por tanto, una monitorización continua pero moderada del estado
de los predecesores.

\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{imagenes/new_model_transmission_schedule}
    \caption{Schedule de transmisión por vehículo para el modelo dinámico con
    \textit{jerk} limitado.}
    \label{fig:new_tx_schedule}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{imagenes/paper_model_transmission_schedule}
    \caption{Schedule de transmisión por vehículo para el modelo cinemático original.}
    \label{fig:paper_tx_schedule}
\end{figure}


En el modelo cinemático de referencia, en cambio, se observan densidades mucho mayores de
transmisiones, especialmente en los vehículos más lejanos al líder. Esto se debe a que los errores de espaciamiento son más abruptos, condición que se acumula hacia la cola del pelotón y obliga a transmisiones más frecuentes para corregir desviaciones repentinas. El agendamiento NOMA, aunque eficiente, no logra distribuir las transmisiones de forma tan uniforme, resultando en intervalos irregulares y picos de actividad que reflejan la naturaleza más errática del error.



La Tabla~\ref{tab:comm_cost} resume las métricas de comunicación agregadas. A pesar de
mostrar mejor desempeño en términos de estabilidad y error, el modelo dinámico con
\textit{jerk} limitado utiliza menos recursos de comunicación: el número total de
paquetes transmitidos se reduce aproximadamente en un $19\%$ (de $3264$ a $2664$), y la
energía de transmisión integrada disminuye alrededor de un $25\%$ (de
$9.02\times10^{-6}\,\mathrm{J}$ a $6.79\times10^{-6}\,\mathrm{J}$). Esto indica que el
modelo propuesto no sólo mejora la calidad de seguimiento, sino que lo hace con un costo
de comunicaciones menor, lo que resulta especialmente relevante para escenarios V2V con
recursos espectrales y energéticos limitados.


\begin{table}[H]
    \centering
    \caption{Métricas agregadas de costo en comunicaciones para ambos modelos.}
    \label{tab:comm_cost}
    \begin{tabular}{lcc}
        \hline
        \textbf{Modelo} & \textbf{Paquetes acumulados} & \textbf{Energía integrada [J]} \\
        \hline
        Cinemático (paper) & $3152$ & $9.02\times10^{-6}$ \\
        Dinámico con \textit{jerk} limitado & $2664$ & $6.79\times10^{-6}$ \\
        \hline
    \end{tabular}
\end{table}
...
Este trabajo replicó la arquitectura de control NOMA-V2V de \cite{He2023} y demostró que los modelos cinemáticos utilizados en la literatura subestiman los requisitos de comunicación y la realidad física del control.

La propuesta de un modelo dinámico limitado por \textit{jerk} evidencia que:
\begin{enumerate}
    \item NOMA es una tecnología habilitadora robusta que permite absorber este exceso de carga de comunicación sin saturar el canal, validando su uso en VCPS de alta fidelidad.
    \item Se redujo el costo de comunicación total en un 25 \%, sugiriendo que modelos más realistas pueden optimizar el uso del canal al evitar transmisiones innecesarias principalmente debido a la sobreestimación del error y a cambios mucho mas suaves.
    \item El modelo dinámico con \textit{jerk} limitado mejora la estabilidad del pelotón, reduciendo el error de espaciamiento promedio y maximo 
\end{enumerate}
Futuras líneas de investigación incluyen la validación experimental en bancos de prueba vehiculares, la introducción de modelos de canal más complejos y la exploración de estrategias de aprendizaja de maquina para optimizar el co-diseño control-comunicación.
